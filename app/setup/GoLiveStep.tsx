...
import SimulatedTestCallModal from "@/components/SimulatedTestCallModal";
...
function GoLiveStep({ form, prevStep }: GoLiveStepProps) {
  ...
  const handleTestCall = async () => {
    ...
    try {
      const res = await fetch("/api/setup/test-call", {
        method: "POST",
      });
      if (res.ok) {
        setTestCallStatus("Test call successful!");
      } else {
        setTestCallStatus("Test call failed. Please try again.");
      }
    } catch (error) {
      setTestCallStatus("Test call failed. Please try again.");
    }
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="flex justify-between gap-2">
          ...
          <Button onClick={handleGoLive} disabled={paymentSuccess}>
